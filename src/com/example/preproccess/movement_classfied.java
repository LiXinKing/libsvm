...
//		private boolean train_predict;//训练和预测的格式不一样，true是预测，false是训练
		public movement_classfied(String path,int window_length,int window_shift) throws IOException{
		...
		public movement_classfied(ArrayList<float[]> floatcollectArray,int window_length,int window_shift) throws IOException{
		...
		private void Data_predict(boolean train_predict) throws IOException{
			String pathString;
			if(train_predict)pathString="//sdcard/train/train_startpoint.txt";
			else pathString="//sdcard/train/predict_startpoint.txt";
			int length;
			...
			for (int i = 0; i < length; i++) {
				...
				if(i==(length-1)){end=floatcollectArray.size();}
				List<float[]> floatinoutlist=floatcollectArray.subList(i*window_shift, end);
				...
				float[] accxmeanshift_extract=translatedata.accxmeanshift_extract(floatinout, train_predict);
				float[] grymean_extract=translatedata.grymean_extract(floatinout, train_predict);
				try {
					FileOutputStream train_model=new FileOutputStream(pathString,true);
					float[] conn = 
					new float[accxmeanshift_extract.length + grymean_extract.length];
		...
		public ArrayList<float[]> Data_splited_return() throws IOException{
			...
			translatedata translatepredict=new translatedata("//sdcard/train/train_startpoint_pd.txt");
			...
			for (int i = 0; i < floatcollectArraypre.size(); i++) {
				...
				if(sublistflag){
					...
					List<float[]> floatcollectArrayreal 
								=floatcollectArray.subList(movementflag*window_shift, window_length+(i-1)*window_shift);
			...
			if(j%2==1){
				...
				ArrayList<float[]> floatinoutpreArrayList=new ArrayList<float[]>();
				List<float[]> floatcollectArrayreal 
							=floatcollectArray.subList(movementflag*window_shift, floatcollectArray.size());
				int listlen=floatcollectArrayreal.size();
				for(int list=0;list<listlen;list++)
				{
					floatinoutpreArrayList.add(floatcollectArrayreal.get(list));
		...
		public void Data_splited() throws IOException
		{
			 int movementflag=0;
			 boolean sublistflag=false;
			translatedata translatepredict=new translatedata("//sdcard/train/predict_startpoint_pd.txt");
			translatepredict.file_array();
			ArrayList<float[]> floatcollectArraypre=translatepredict.floatcollectArray;
			int j=0;
			for (int i = 0; i < floatcollectArraypre.size(); i++) {
				...
				if(sublistflag){
					sublistflag=false;
					j++;
					splitting(movementflag*window_shift, window_length+(i-1)*window_shift);  
		...
		private void splitting(int start,int end) throws IOException {
			...
			ArrayList<float[]> floatinoutpreArrayList=new ArrayList<float[]>();
			List<float[]> floatcollectArrayreal 
						=floatcollectArray.subList(start, end);
			int listlen=floatcollectArrayreal.size();
			for(int list=0;list<listlen;list++)
			{
				floatinoutpreArrayList.add(floatcollectArrayreal.get(list));
			...
			FileOutputStream predict_real=new FileOutputStream("//sdcard/train/predict_real",true);	
