...
/*		 float []tt0={1,2,3};
		 float []tt1={2,3,4};
			ArrayList<float[]> ttList=new ArrayList<float[]>();
			ttList.add(tt0);
			ttList.add(tt1);
			generate_matrix(ttList);*/

		 placeString=place;
/*		File dirFile=new File(placeString);
			  if (!dirFile.exists()) {
				  dirFile.mkdirs();
				  }*/
...
//				printFloatArrarylist(floatinout_forsplited,"/sdcard/112233");
		...
		private void printFloatArrarylist(ArrayList<float[]> floatinout,String path) throws IOException
		{
	...
	private int min_eigenvalues(float[][] eigenvalues) {
			// TODO Auto-generated method stub
		if((eigenvalues[1][1]>eigenvalues[0][0])&&(eigenvalues[2][2]>eigenvalues[0][0]))
		return 0;
	...
	private float[][] generate_matrix_9(float[] bufferx,float[] buffery,float[] bufferz) {
	...
	private float[] accxmean_extract(ArrayList<float[]> floatcollectArray,Boolean flag)
	{	
		float sumx=0;
		float sumy=0;
		float sumz=0;
	...
	private int eigenvalues_extarct(float a[][],int n,float v[][],float eps,int jt) {	
